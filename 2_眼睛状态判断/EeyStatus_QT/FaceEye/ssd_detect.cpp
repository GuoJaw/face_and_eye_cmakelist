...
{    
    ...
    {              
        ...
        const int p1 =  static_cast<int>(d[3] * img.cols -20);
        const int p2 =  static_cast<int>(d[4] * img.rows);
        const int p3 =  static_cast<int>(d[5] * img.cols+30);
...
{    
    ...
    {              
	    ...
	    //矩形框
            ...
            std::string title = label + ": " + flt2str(score);
            cv::putText(Faceimg, title, cv::Point(point1.x, point2.y+15), cv::FONT_ITALIC, 0.3, cv::Scalar(0, 255, 255), 1,1);  //画字坐标-->cv::Point(point1.x, point2.y+15),
            cv::rectangle(Faceimg, point1, point2, cv::Scalar(0, 255, 255), 1);
