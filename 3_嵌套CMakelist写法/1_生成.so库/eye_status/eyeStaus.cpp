...
const int isMobilenet_ = 1;

DEFINE_string(mean_file_, "", "The mean file used to subtract from the input image.");


#if isMobilenet_
DEFINE_string(mean_value_, "127", "If specified, can be one value or can be same as image channels"
    " - would subtract from the corresponding channel). Separated by ','."
    "Either mean_file_ or mean_value_ should be provided, not both.");
#else
DEFINE_string(mean_value_, "104,117,123", "If specified, can be one value or can be same as image channels"
    " - would subtract from the corresponding channel). Separated by ','."
    "Either mean_file_ or mean_value_ should be provided, not both.");
#endif

DEFINE_string(file_type_, "image", "The file type in the list_file. Currently support image and video.");
DEFINE_double(confidence_threshold_, 0.7, "Only store detections with score higher than the threshold.");


string mean_file_ = FLAGS_mean_file_;
string mean_value_ = FLAGS_mean_value_;
string file_type_ = FLAGS_file_type_;

float confidence_threshold_ = FLAGS_confidence_threshold_;
...
int test_eyeStates()
{
    ...
    string mean_file__classifer    = "../model/bvlc_alexnet/mean.binaryproto";  //均值文件
    string label_file_classifer   = "../model/bvlc_alexnet/labels.txt";  //标签文件：labels.txt

    //
    std::cout << " confidence_threshold_=" << confidence_threshold_ << std::endl;

    // 加载网络
    Detector detector_face;
    detector_face.Set(model_file_face, weights_file_face, mean_file_, mean_value_, isMobilenet_);
    // 加载网络
    Detector detector_eye;
    detector_eye.Set(model_file_eye, weights_file_eye, mean_file_, mean_value_, isMobilenet_);
    // 加载网络
    Classifier classifier(model_file_classifer, trained_file_classifer, mean_file__classifer, label_file_classifer);
    ...
    while (true)
    {
      ...
      cv::Mat FaceImg;
      detector_face.Postprocess_Face(img, confidence_threshold_, detections_face,FaceImg);   //SSD检测
      ...
      if (!FaceImg.empty()) //如果检测到脸
      {
                      ...
                      detector_eye.Postprocess_Eye(FaceImg, confidence_threshold_, detections_eye,EyeVec);   //SSD检测
